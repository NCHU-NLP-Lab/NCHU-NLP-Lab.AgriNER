...
class NERDataset(Dataset):
    def __init__(self, words, labels, config, is_train=True, word_pad_idx=0, label_pad_idx=-1):
        tokenizer = None
        ...
        self.dataset = self.preprocess(words, labels, is_train)
    ...
    def gen_words(self,words,is_train):
        if is_train:
            return [item for token in words for item in token]
        return ['[CLS]'] + [item for token in words for item in token]

    def preprocess(self, origin_sentences, origin_labels, is_train):
        """
        Maps tokens and tags to their indices and stores them in the dict data.
        examples:
            word:['[CLS]', '浙', '商', '银', '行', '企', '业', '信', '贷', '部']
        ...
        for line in origin_sentences:
            ...
            #
            # origin_label = origin_labels[ori_idx]
            #
            # for i in range(len(origin_label)):
            #     if origin_label[i] != 'O':
            #         if origin_label[i][0] == 'B' or origin_label[i][0] == 'S':
            #             words = words + ['[SEP]']
            #         words = words + [words[i]]
            #
            # words = ['[CLS]'] + words
            ...
            words = ['[CLS]'] + [item for token in words for item in token]
            # words = self.gen_words(words, is_train)
            # if is_train:
            #     # words = [item for token in words for item in token]
            #
            #     origin_label = origin_labels[ori_idx]
