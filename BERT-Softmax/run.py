...
def test():
    ...
    for label in config.labels:
        # logging.info("f1 score of {}: {}".format(label, val_f1_labels[label][0],val_f1_labels[label][1],val_f1_labels[label][2]))
        logging.info("Label:{} => f1 : {}, P : {}, R : {}".format(label, val_f1_labels[label][0],val_f1_labels[label][1],val_f1_labels[label][2]))
...
def run():
    ...
    if (config.use_model == 'bert'):
        ...
        model = BertNER.from_pretrained("voidful/albert_chinese_base", num_labels=len(config.label2id))
