...
from transformers.optimization import get_cosine_schedule_with_warmup, AdamW,get_linear_schedule_with_warmup
...
def test():
    ...
    logging.info("test loss: {}, f1 score: {}, P score:{}, R score: {}".format(val_metrics['loss'], val_f1, p, r))
    val_f1_labels = val_metrics['f1_labels']
    for label in config.labels:
        logging.info(
            "Label:{} => f1 : {}, P : {}, R : {}".format(label, val_f1_labels[label][0], val_f1_labels[label][1],
                                                         val_f1_labels[label][2]))
...
def run():
    ...
    if (config.use_model == 'bert'):
        ...
        model = BertNER.from_pretrained(config.distilbert_model, num_labels=len(config.label2id))
    elif (config.use_model == 'albert'):
        # model = BertNER.from_pretrained(config.albert_model, num_labels=len(config.label2id))
        model = BertNER.from_pretrained(config.albert_model, num_labels=len(config.label2id))
    ...
    if config.full_fine_tuning:
        ...
        optimizer_grouped_parameters = [
            ...
            {'params': model.crf.parameters(), 'lr': config.learning_rate * config.CRF_rate}
    ...
    # scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=(config.epoch_num // 10) * train_steps_per_epoch,
    #                                                  num_training_steps=config.epoch_num * train_steps_per_epoch)
