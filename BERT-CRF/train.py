...
from transformers import BertTokenizer,AutoTokenizer,AutoConfig,DistilBertModel
...
def train(train_loader, dev_loader, model, optimizer, scheduler, model_dir):
    ...
    for epoch in range(1, config.epoch_num + 1):
        ...
        logging.info(
            "Epoch: {}, dev loss: {}, f1 score: {}, P score:{}, R score: {}".format(epoch, val_metrics['loss'], val_f1,
                                                                                    p, r))
...
def evaluate(dev_loader, model, mode='dev'):
    ...
    if mode == 'test':
        ...
        elif (config.use_model == 'roberta'):
            tokenizer = BertTokenizer.from_pretrained(config.roberta_model, do_lower_case=True)
        elif (config.use_model == 'distilbert'):
            # tokenizer = BertTokenizer.from_pretrained("hfl/chinese-electra-base-discriminator", do_lower_case=True)
            tokenizer = BertTokenizer.from_pretrained(config.distilbert_model, do_lower_case=True)
        elif (config.use_model == 'albert'):
            # tokenizer = BertTokenizer.from_pretrained(config.albert_model, do_lower_case=True)
            tokenizer = BertTokenizer.from_pretrained(config.albert_model, do_lower_case=True)
            # tokenizer = AutoTokenizer.from_pretrained("voidful/albert_chinese_base", do_lower_case=True)
    ...
    metrics = {}
    if mode == 'dev':
        f1, p, r = f1_score(true_tags, pred_tags, mode)
        metrics['f1'] = f1
        metrics['p'] = p
        metrics['r'] = r
    else:
...
if __name__ == "__main__":
    ...
    model_config = AutoConfig.from_pretrained("hfl/chinese-electra-base-discriminator", num_labels=8)
    print(model_config)
    model = DistilBertModel.from_pretrained("hfl/chinese-electra-base-discriminator")
    print(model)